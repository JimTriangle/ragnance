...
const AdminPage = () => {
    ...
    const saveUser = async () => {
        try {
            if (selectedUser) {
                await api.put(`/admin/users/${selectedUser.id}`, { email: userData.email, role: userData.role });
                toast.current.show({ severity: 'success', summary: 'Succès', detail: 'Utilisateur modifié' });
            } else {
